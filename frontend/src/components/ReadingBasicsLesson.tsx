...
export function ReadingBasicsLesson({ onBack }: ReadingBasicsLessonProps) {
  ...
  const [selectedLetter, setSelectedLetter] =
    useState<(typeof letterSounds)[number] | null>(null);
  ...
  const [currentStoryId, setCurrentStoryId] = useState<number>(tinyStories[0].id);
  ...
  // Calculate progress based on interaction
const calculateProgress = () => {
  ...
  const handleLetterClick = (letter: (typeof letterSounds)[number]) => {
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-xl p-4 mb-4">
                      <div className="flex flex-wrap gap-2 justify-center">
                        {story.words.map((word, index) => (
                          <button
                            key={index}
                            onClick={() => handleWordClick(story.id, index)}
