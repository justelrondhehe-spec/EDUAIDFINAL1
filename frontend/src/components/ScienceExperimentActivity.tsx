...
export function ScienceExperimentActivity({
  onBack,
}: ScienceExperimentActivityProps) {
  const { completeActivity, activityScores } = useApp();

  // ⚠️ MAKE SURE THIS MATCHES YOUR DB / SEED
  const activityId = 4;
  ...
  const existingScore =
    ...
    activityScores?.[String(activityId)] ??
    null;
  ...
  // 🔗 AUTO-SYNC WITH BACKEND WHEN DONE (and open popup once)
  useEffect(() => {
    if (!allModulesComplete || hasSubmitted) return;

    const score = calculateProgress(); // should be 100 when all correct/complete
    const maxScore = 100;

    completeActivity(activityId, score, maxScore)
      .catch((err: unknown) => {
        console.warn("Failed to complete Science Experiment activity:", err);
      })
      .finally(() => {
        setHasSubmitted(true);
  ...
  const handleBodyPartClick = (partId: string) => {
    ...
    if (isCorrect && currentBodyMapQuestion < bodyMapQuestions.length - 1) {
      setTimeout(() => setCurrentBodyMapQuestion((prev) => prev + 1), 1500);
    }
  };
  ...
  const handleSafetyAnswer = (isSafe: boolean) => {
    ...
    if (currentSafetyCard < safetyCards.length - 1) {
      setTimeout(() => setCurrentSafetyCard((prev) => prev + 1), 2000);
    }
  };
  ...
  const handleResetAll = () => {
    ...
    // Do NOT touch hasSubmitted: backend completion should stay
  };

  const currentQuestion = bodyMapQuestions[currentBodyMapQuestion];
  const bodyMapAnswer = bodyMapAnswers[currentBodyMapQuestion];
  const isBodyMapCorrect = bodyMapAnswer === currentQuestion?.correctAnswer;
            ...
            <div className="grid grid-cols-3 gap-4">
              {matchObjects.map((obj) => {
                ...
                return (
                  ...
                  >
                    <div className="text-5xl mb-3 text-center">{obj.emoji}</div>
