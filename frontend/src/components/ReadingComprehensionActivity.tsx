...
export function ReadingComprehensionActivity({
  onBack,
}: ReadingComprehensionActivityProps) {
  const { completeActivity, activityScores } = useApp();

  // 🚨 MUST MATCH your DB / seed activity id
  const activityId = 3;

  // Module 1: The Sound Match (Phonics)
  const [soundMatchAnswers, setSoundMatchAnswers] = useState<
  ...
  const [traceCompleted, setTraceCompleted] = useState<Set<number>>(
    () => new Set<number>()
  );
  ...
  // prevent double submission to backend
  const [hasSubmitted, setHasSubmitted] = useState(false);

  // local completion popup
  const [showCompletionDialog, setShowCompletionDialog] = useState(false);

  const existingScore =
    activityScores[activityId] ?? activityScores[String(activityId)] ?? null;

  // If already completed in backend, don't resubmit
  useEffect(() => {
      ...
      options: [
        {
          ...
          name: "Ball",
          correct: true,
          feedback: "B is for Ball! /buh/ /buh/ Ball.",
        },
        {
          emoji: "🍎",
          name: "Apple",
          correct: false,
          feedback: "Try again! Apple starts with A.",
        },
        {
          emoji: "☀️",
          name: "Sun",
          correct: false,
          feedback: "Try again! Sun starts with S.",
        },
      ],
    },
    {
      ...
      options: [
        {
          ...
          name: "Fish",
          correct: false,
          feedback: "Try again! Fish starts with F.",
        },
        {
          emoji: "🌙",
          name: "Moon",
          correct: true,
          feedback: "M is for Moon! /muh/ /muh/ Moon.",
        },
        {
          emoji: "📦",
          name: "Box",
          correct: false,
          feedback: "Try again! Box starts with B.",
        },
      ],
    },
    {
      ...
      options: [
        {
          ...
          name: "Hat",
          correct: false,
          feedback: "Try again! Hat starts with H.",
        },
        {
          emoji: "☀️",
          name: "Sun",
          correct: true,
          feedback: "S is for Sun! /sss/ /sss/ Sun.",
        },
        {
          emoji: "🦁",
          name: "Lion",
          correct: false,
          feedback: "Try again! Lion starts with L.",
        },
      ],
    },
  ];
  ...
  const trainWords = [
    {
      ...
      sounds: ["C", "A", "T"],
      image: "🐱",
      meaning: "A furry pet that meows",
    },
    {
      word: "DOG",
      sounds: ["D", "O", "G"],
      image: "🐕",
      meaning: "A four-legged friend",
    },
    {
      word: "SUN",
      sounds: ["S", "U", "N"],
      image: "☀️",
      meaning: "Bright light in the sky",
    },
  ];
  ...
  const calculateProgress = () => {
    const module1 =
      (Object.keys(soundMatchAnswers).length / soundScenarios.length) * 25;
    const module2 =
      (Object.keys(trainAnswers).length / trainWords.length) * 25;
    const module3 = (traceCompleted.size / traceLetters.length) * 25;
    const module4 =
      (Object.keys(storyAnswers).length / tinyStories.length) * 25;
    return Math.round(module1 + module2 + module3 + module4);
  };

  const progressPercent =
    existingScore?.completed && typeof existingScore.score === "number"
      ? existingScore.score
      : calculateProgress();
  ...
  const handleReset = () => {
    ...
    setTrainAnswers({});
    setTraceCompleted(new Set<number>());
    setStoryAnswers({});
  ...
  const handleSoundMatch = (optionName: string) => {
    ...
    if (option) {
      setSoundMatchAnswers({
        ...soundMatchAnswers,
        [currentSoundScenario]: optionName,
      });

      if (option.correct && currentSoundScenario < soundScenarios.length - 1) {
        setTimeout(
          () => setCurrentSoundScenario((prev) => prev + 1),
          2000
        );
      }
    }
  ...
  // Trace handlers
  const handleTrace = () => {
    if (isTracing || traceCompleted.has(currentTraceLetter)) return;

    setIsTracing(true);
    setTimeout(() => {
      const newCompleted = new Set<number>(traceCompleted);
      ...
      if (currentTraceLetter < traceLetters.length - 1) {
        setTimeout(
          () => setCurrentTraceLetter((prev) => prev + 1),
          1500
        );
      }
    }, 2000);
  ...
  const handleStoryComplete = () => {
    ...
    if (currentStory < tinyStories.length - 1) {
      setTimeout(() => setCurrentStory((prev) => prev + 1), 2000);
    }
  };
  ...
  const selectedOption = currentSound?.options.find(
    (o) => o.name === soundAnswer
  );
  ...
  // ---------- AUTO-SUBMIT & POPUP WHEN DONE ----------

  // Show popup when they finish all modules
  useEffect(() => {
    if (allModulesComplete && !showCompletionDialog) {
      setShowCompletionDialog(true);
    }
  }, [allModulesComplete, showCompletionDialog]);

  // Save to backend once
  useEffect(() => {
    if (!allModulesComplete || hasSubmitted) return;

    const score = calculateProgress(); // should be 100 when allModulesComplete is true
    const maxScore = 100;

    completeActivity(activityId, score, maxScore).catch((err) => {
      console.warn("Failed to complete Reading Comprehension activity:", err);
    });

    setHasSubmitted(true);
  }, [allModulesComplete, hasSubmitted, completeActivity]);
          ...
          <div className="bg-gradient-to-br from-green-600 to-emerald-600 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-white/90 mb-4">
                Complete 4 fun modules to become a Reading Star! Match sounds,
                blend words, trace letters, and build stories.
              </p>
                ...
                <div className="text-center space-y-6">
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-2xl p-12 border-4 border-dashed border-yellow-300 dark:border-yellow-700">
                    {!isLetterTraced ? (
                      <>
                        <div
                          className={`text-9xl mb-6 transition-all ${
                            isTracing ? "opacity-70" : "opacity-100"
                          }`}
                        >
                          {currentLetter.letter}
                        </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <p className="text-green-800 dark:text-green-300 font-bold text-2xl">
                          {currentLetter.letter} turns into{" "}
          ...
          {allModulesComplete && (
            <div className="bg-gradient-to-br from-yellow-400 to-orange-500 rounded-2xl p-8 text-center shadow-2xl">
              ...
              <p className="text-white/90 text-xl">
                You&apos;ve completed all 4 modules! You&apos;re an amazing
                reader! 📚
              </p>
            </div>
          )}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-center">
              Great job! You finished the Reading Comprehension activity.
            </DialogDescription>
          </DialogHeader>

          <div className="mt-4 space-y-3 text-center">
            <p className="text-lg font-semibold">
              Final Score: {progressPercent} / 100
            </p>
            <p className="text-sm text-slate-500">
              Your progress has been saved. You can come back and practice
              anytime.
            </p>
          </div>
