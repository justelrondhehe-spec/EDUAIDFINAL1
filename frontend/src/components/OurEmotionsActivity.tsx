// frontend/src/components/OurEmotionsActivity.tsx
...
export function OurEmotionsActivity({ onBack }: OurEmotionsActivityProps) {
  ...
  /**
   * This MUST match the `id` of "Our Emotions Activity"
   * in your activities seed / database.
   *
   * Make sure activitiesSeed.json has something like:
   * {
   *   "id": 7,
  ...
  // Modules state
  const [faceMatchAnswers, setFaceMatchAnswers] = useState<
    Record<number, string>
  >({});
  const [currentFaceScenario, setCurrentFaceScenario] = useState(0);

  const [detectiveAnswers, setDetectiveAnswers] = useState<
    Record<number, string>
  >({});
  ...
  // backend + dialog control
  const [hasSubmitted, setHasSubmitted] = useState(false);
  const [showCompletionDialog, setShowCompletionDialog] = useState(false);

  // if the activity is already completed (from backend), don't resubmit
  useEffect(() => {
    if (existingScore?.completed) {
      setHasSubmitted(true);
      setShowCompletionDialog(false); // already done earlier
    }
  }, [existingScore]);
  ...
  const detectiveScenarios = [
    ...
    },
    {
      ...
      scene: "⛈️🌩️",
      description:
        "A window showing dark clouds and a lightning bolt. The child is hiding under a blanket.",
      ...
      toolOptions: [
        {
          ...
          emoji: "🌬️",
          name: "Deep Breaths",
          feedback: "Breathe in... Breathe out. I am calm.",
        },
    ...
    },
    {
      ...
      toolOptions: [
        { id: "breathe", emoji: "🌬️", name: "Deep Breaths", feedback: "" },
        {
          ...
          emoji: "🧸",
          name: "Hug a Teddy",
          feedback: "Hugs help us feel safe.",
        },
        { id: "light", emoji: "🕯️", name: "Night Light", feedback: "" },
        { id: "move", emoji: "🏃", name: "Run & Jump", feedback: "" },
      ],
    },
    {
      ...
      toolOptions: [
        ...
        { id: "hug", emoji: "🧸", name: "Hug a Teddy", feedback: "" },
        {
          ...
          emoji: "🕯️",
          name: "Night Light",
          feedback: "The light pushes the dark away.",
        },
        { id: "move", emoji: "🏃", name: "Run & Jump", feedback: "" },
      ],
    },
    {
      ...
      toolOptions: [
        ...
        { id: "light", emoji: "🕯️", name: "Night Light", feedback: "" },
        {
          ...
          emoji: "🏃",
          name: "Run & Jump",
          feedback: "Move your body to get the wiggles out!",
        },
      ],
    },
  ];
  ...
  const calculateProgress = () => {
    const module1 =
      (Object.keys(faceMatchAnswers)
        ...
        faceScenarios.length) *
      25;

    const module2 =
      (Object.keys(detectiveAnswers)
        .filter((key) => {
          const idx = parseInt(key);
        ...
        detectiveScenarios.length) *
      25;

    const module3 =
      (Object.keys(toolMatches)
        .filter((feeling) => {
    ...
    const module4 = module3; // auto complete when module3 done

    return Math.round(module1 + module2 + module3 + module4);
  };

  const currentProgress = existingScore?.completed
    ? 100
    : calculateProgress();
  ...
  const handleFaceMatch = (optionName: string) => {
    ...
    setFaceMatchAnswers({
      ...faceMatchAnswers,
      [currentFaceScenario]: optionName,
    });

    if (option?.correct && currentFaceScenario < faceScenarios.length - 1) {
      setTimeout(() => setCurrentFaceScenario((prev) => prev + 1), 2000);
    }
  };
  ...
  const handleDetectiveAnswer = (optionName: string) => {
    ...
    setDetectiveAnswers({
      ...detectiveAnswers,
      [currentDetectiveScenario]: optionName,
    });

    if (
      option?.correct &&
      currentDetectiveScenario < detectiveScenarios.length - 1
    ) {
      setTimeout(() => setCurrentDetectiveScenario((prev) => prev + 1), 2000);
    }
  };
  ...
  const selectedFaceOption = currentFace?.options.find(
    (o) => o.name === faceAnswer
  );
  ...
  const selectedDetectiveOption = currentDetective?.options.find(
    (o) => o.name === detectiveAnswer
  );
  ...
  const allModulesComplete =
    ...
    calmingTools.every(
      (tool) => toolMatches[tool.feeling] === tool.correctTool
    );

  // ---------- auto submit to backend + open popup ----------
  useEffect(() => {
    if (!allModulesComplete || hasSubmitted) return;

    const score = 100; // if allModulesComplete, progress is 100
    const maxScore = 100;

    completeActivity(activityId, score, maxScore)
      .catch((err) => {
        console.warn("Failed to complete Our Emotions Activity:", err);
      })
      .finally(() => {
        setHasSubmitted(true);
        setShowCompletionDialog(true);
      });
  }, [allModulesComplete, hasSubmitted, activityId, completeActivity]);
  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-br from-pink-50 via-rose-50 to-red-50 dark:from-slate-900 dark:via-pink-900/20 dark:to-rose-900/20 p-6">
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="bg-gradient-to-br from-pink-600 to-rose-600 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-white/90 mb-4">
                Complete 4 fun modules to become an Emotion Expert! Match faces,
                detect feelings, and learn calming tools.
              </p>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-lg border border-slate-200 dark:border-slate-700">
            ...
            {currentFace && (
              <div
                className={`${currentFace.bgColor} rounded-xl p-6 border-2 ${
                  selectedFaceOption?.correct
                    ? "border-green-500"
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-lg border border-slate-200 dark:border-slate-700">
            ...
            <div className="space-y-6">
              {calmingTools.map((tool, toolIdx) => {
                ...
                const selectedTool = tool.toolOptions.find(
                  (t) => t.id === matched
                );
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          {tool.toolOptions.map((option, optIdx) => (
                            <button
                              key={optIdx}
                              onClick={() =>
                                handleToolMatch(tool.feeling, option.id)
                              }
                        ...
                        >
                          ...
                          >
                            {isCorrect
                              ...
                              : "Try again! Think about what helps with this feeling."}
                          </p>
                        </div>
                      )}
                    </div>
          ...
          {allModulesComplete && (
            <div className="bg-gradient-to-br from-yellow-400 to-orange-500 rounded-2xl p-8 text-center shadow-2xl">
              ...
              <p className="text-white/90 text-xl">
                You&apos;ve completed all modules! You understand feelings and
                know how to stay calm! ❤️
              </p>
            </div>
          )}
        </div>
      ...
      {/* Completion Popup (like Shapes & Colors) */}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-center">
              Perfect! You&apos;ve completed all parts of the Our Emotions
              activity.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-900/20 dark:to-green-900/20 border border-emerald-200 dark:border-emerald-800/30 rounded-xl p-4">
              ...
              <div className="text-sm text-slate-600 dark:text-slate-400 text-center">
                You are now an Emotion Expert! Great job using your feeling and
                calming tools.
              </div>
            </div>
          </div>
          <div className="flex flex-col gap-2 mt-4">
            <button
              ...
              className="inline-flex items-center justify-center rounded-xl px-4 py-2 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white font-medium"
            >
              Back to Activities
            </button>
            <button
              onClick={() => setShowCompletionDialog(false)}
              className="inline-flex items-center justify-center rounded-xl px-4 py-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 font-medium"
            >
              Try Again
            </button>
          </div>
